...
class internationalTransport extends React.Component {
  render() {
    ...
    return (
      <Layout>
        <Seo title={intl.formatMessage({ id: "services.private-transport.meta-title" })} description={intl.formatMessage({ id: "services.private-transport.meta-description" })} siteLocale={intl.formatMessage({ id: "general.locale" })} />
...
export const pageQuery = graphql`
  query HomeQuery($language: String) {
    allContentfulTextContent(filter: {websiteSection: {eq: "private-transport"}, node_locale: { eq: $language } }) {
      nodes {
        title
        textContent {
          raw
